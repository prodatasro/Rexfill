...
const LoginDialog: FC<LoginDialogProps> = ({ isOpen, onClose }) => {
  ...
  return createPortal(
    ...
    >
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-md w-full p-6 relative animate-in fade-in zoom-in-95 duration-200">
        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 p-1 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
          aria-label={t('confirmDialog.cancel')}
        >
          <X className="w-5 h-5" />
        </button>
          ...
          >
            ...
            <div className="text-left">
              <div className="font-medium text-slate-900 dark:text-white">
                {t('loginDialog.internetIdentity')}
              </div>
              <div className="text-sm text-slate-500 dark:text-slate-400">
                {t('loginDialog.internetIdentityDesc')}
              </div>
            </div>
          </button>
